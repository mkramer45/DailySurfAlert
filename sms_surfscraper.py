...
############# All in one function that scrapes surf data & determines if it's a good day for surfing or not.#############
def surf_scraper():
	#Government Buoy URL that includes all of our required surf information.
	...
	# Fetching/Defining data to variables
		...
		# Creating lists of WaveHeights, Wave Intervals, Wind Directions to check against
...
############# Tide Info #############
	...
	tide_urls = [
	#URL that contains all of the tide information for Boston Harbor.
  ...
  # 1 & 2: Boston Tide List + Boston Time List
	...
	# End users will receive SMS messages indicating if the surf conditions in Boston are good OR bad.
	if wave_height not in wave_height_list and wave_interval not in wave_interval_list and wind_direction in wind_direction_list and wind_speed_abbreviated_int < 17:
		...
		print("End.")
		print("\n")
	# If the current conditions are NOT satisfactory based on the above logic, sender is informed the bad news via the else statement below.
	else:
		print("\n")
